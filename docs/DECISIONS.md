...
## 2026-07-20 — SVG image captcha (svg-captcha) instead of math captcha

**Decision:** Replaced the math question captcha (`{a} + {b} = ?`) with an SVG image captcha generated by the `svg-captcha` library.

**Why:**
- SPEC required a captcha with letters and digits, displayed as an image.
- `svg-captcha` is pure JS with zero native dependencies — no Cairo/Pango required (unlike `node-canvas`).
- Produces SVG with noise lines, random colors, and text distortion — visually similar to a raster captcha.
- Answer is still JWT-signed, fully stateless — no DB changes needed.

**What changed:**
- `backend/src/captcha/captcha.service.ts`: `generate()` now uses `svgCaptcha.create()` (5→7 chars, dark background, color, noise lines) instead of `crypto.randomInt(1, 50)` math.
- `frontend/src/types/captcha.ts`: `Captcha` interface changed from `{ token, a, b }` to `{ token, svg }`.
- `frontend/src/components/CommentForm/CaptchaSection.tsx`: renders SVG via `<img src="data:image/svg+xml,...">` instead of `{a} + {b} = ?` text; `inputMode` changed to text.
- `frontend/src/components/CommentForm/hooks/useCaptcha.ts`: type guard checks `svg: string` instead of `a: number, b: number`.
- `frontend/src/components/CommentForm/CommentForm.module.css`: `.captchaImage` styles (border, background, max-width); input widened to 110px.
- `backend/package.json`: added `svg-captcha` dependency.
- `backend/nest-cli.json`: moved `entryFile` to top level (was inside `compilerOptions`, causing `MODULE_NOT_FOUND` at runtime).
